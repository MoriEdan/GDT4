...
//	public Vector3 BottomLeft;
	...
	public GameObject OverlayObjectR;
	...
	void Start()
	{
		if(OverlayObjectR)
		{
			distanceToCamera = (OverlayObjectR.transform.position - Camera.main.transform.position).magnitude;
		}
	}
...
//			Vector3 vUp = TopLeft - BottomLeft;
			...
			int iJointIndexR = (int)TrackedRightHand;
            int iJointIndexL = (int)TrackedLeftHand;
			
			if(manager.IsUserDetected())
			{
				uint userId = manager.GetPlayer1ID();
				
				if(manager.IsJointTracked(userId, iJointIndexR))
				{
					Vector3 posJoint = manager.GetRawSkeletonJointPos(userId, iJointIndexR);
...
//						Vector3 vPosOverlay = backgroundImage.transform.TransformPoint(localPos);
						...
						if(OverlayObjectR)
						{
							...
							OverlayObjectR.transform.position = Vector3.Lerp(OverlayObjectR.transform.position, vPosOverlay, smoothFactor * Time.deltaTime);
						}
					}
				}
                ...
                if (manager.IsJointTracked(userId, iJointIndexL))
                {
                    ...
                    if (posJoint != Vector3.zero)
                    {
                        ...
                        {
                            Vector3 vPosOverlay = Camera.main.ViewportToWorldPoint(new Vector3(scaleX, scaleY, distanceToCamera));
                            OverlayObjectL.transform.position = Vector3.Lerp(OverlayObjectL.transform.position, vPosOverlay, smoothFactor * Time.deltaTime);
                        }
                    }
                }
